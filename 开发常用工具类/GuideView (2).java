package com.wtyt.yzone.util.guide;
...
public class GuideView extends RelativeLayout implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    // TODO: 2017/12/1  移除所有view(验证)
    private void removeViews() {
        guideViewLayout.removeAllViews();
    }

    public void showOnce(int type) {
        if (targetView != null) {
            mContent.getSharedPreferences(TAG, Context.MODE_PRIVATE).edit().putBoolean(generateUniqId( type), true).commit();
        }
    }

    private boolean hasShown(int type) {
        if (targetView == null)
            return true;
        return mContent.getSharedPreferences(TAG, Context.MODE_PRIVATE).getBoolean(generateUniqId(type), false);
    }

    private String generateUniqId( int type) {
        return SHOW_GUIDE_PREFIX + type;
    }
            ...
            if (direction != null) {
                ...
                switch (direction) {
                    ...
                    case SIGN_CENTER_BOTTON:
                        guideViewParams.setMargins(right / 2 + offsetX, bottom + 15 + offsetY, -right - offsetX, -top - offsetY);
                        break;
                    case TAB_SHEQU:
                        this.setGravity(Gravity.BOTTOM);
                        guideViewParams.setMargins(right / 2 + offsetX, offsetY - height + top, -right - offsetX, height - top - offsetY);
                        break;
                    case GUIDE_CHAT:
                        this.setGravity(Gravity.RIGHT);
                        guideViewParams.setMargins(offsetX - width + left, bottom + 15 + offsetY, (width - left - offsetX) / 2, -bottom - offsetY);
                        break;
                    case GUIDE_TOP:
                        this.setGravity(Gravity.RIGHT | Gravity.BOTTOM);
                        guideViewParams.setMargins(offsetX - width + left, offsetY - height + top, (width - left - offsetX) + 10, (height - top - offsetY) / 2);
                        break;
                    case GUIDE_POST:
                        this.setGravity(Gravity.RIGHT | Gravity.BOTTOM);
                        guideViewParams.setMargins(offsetX - width + left, offsetY - height, (width - left - offsetX) / 2, (height - top - offsetY) - deviation);
                        break;
                    case GUIDE_MOVE_IMG:
                        this.setGravity(Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL);
                        guideViewParams.setMargins(offsetX, offsetY - height + top, -offsetX, (height - top - offsetY) / 2);
                        break;
                    default:
                }
            } else {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (myShape != null) {
            ...
            switch (myShape) {
                ...
                case RECTANGLE://矩形
                    int[] size = getTargetViewSize();
                    oval.left = center[0] - size[0] / 2;                                 //左边
                    oval.top = center[1] - size[1] / 2;                                  //上边
                    oval.right = center[0] + size[0] / 2;                                   //右边
                    oval.bottom = center[1] + size[1] / 2;                              //下边
    ...
    public enum Direction {
        ...
        RIGHT_TOP, RIGHT_BOTTOM, SIGN_CENTER_BOTTON, TAB_SHEQU, GUIDE_CHAT, GUIDE_TOP, GUIDE_POST, GUIDE_MOVE_IMG
    }
     ...
     */
    public enum MyShape {
        CIRCULAR, ELLIPSE, RECTANGULAR, RECTANGLE
    }
    ...
    public static class Builder {
        ...
        public Builder setDeviation(int x) {
            guiderView.setDeviation(x);
            return instance;
        }
        ...
        public Builder showOnce(int type) {
            guiderView.showOnce(type);
            return instance;
        }
        ...
        public Builder setOnLongClickListener(final OnLongClickListener callback) {
